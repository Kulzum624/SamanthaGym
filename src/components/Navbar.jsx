        ...
        <nav className={`fixed w-full z-40 transition-all duration-300 ${scrolled ? 'bg-white shadow-md' : 'bg-white/95 backdrop-blur-sm'} ${scrollDir === 'down' ? 'top-0' : 'top-12'}`}>
            <div className="max-w-7xl mx-auto px-4 lg:px-8 flex justify-between items-center h-[72px]">
                ...
                <Link to="/" className="flex items-center">
                    <span className="font-poppins font-black text-[22px] tracking-tight text-primary-dark [word-spacing:10px] uppercase relative">
                        KULZUM M.
                ...
                <div className="flex items-center gap-4">
                    <Link to="/pricing-table" className="hidden sm:flex bg-primary-lime text-black font-poppins font-bold uppercase text-[13px] px-6 py-2.5 tracking-wider hover:brightness-110 transition-all hover:-translate-y-0.5 hover:shadow-lg items-center gap-2">
                        JOIN NOW
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    ...
                    >
                        <div className="px-4 py-4 space-y-1 overflow-y-auto max-h-[calc(100vh-72px)]">
                            {navLinks.map((item, idx) => (
                                <div key={idx}>
                                    <Link
                                        to={item.path}
